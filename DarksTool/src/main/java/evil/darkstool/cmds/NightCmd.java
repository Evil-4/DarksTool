...
public class NightCmd extends Command {
    ...
    public NightCmd(String name, DarksTool plugin) {
        super(name);
        this.plugin = plugin;
    }
    @Override
    public boolean execute(@NotNull CommandSender sender, @NotNull String arg2, @NotNull String[] args) {
        if (sender instanceof Player) {
            ...
            if (p.hasPermission("darksmc.night")) {
                p.getWorld().setTime(14000);
                p.getWorld().setWeatherDuration(0);
                p.getWorld().setStorm(false);
                p.sendMessage("§a✔ §x§7§9§F§F§5§5Zmieniłeś porę dnia na §x§3§0§D§F§0§0Noc §x§7§9§F§F§5§5i zresetowałeś pogode");
            } else {
