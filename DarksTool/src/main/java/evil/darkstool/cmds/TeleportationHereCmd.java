...
public class TeleportationHereCmd extends Command {

    private DarksTool plugin;
    public TeleportationHereCmd(String name, DarksTool plugin) {
        super(name);
        this.plugin = plugin;
    }
    @Override
    public boolean execute(@NotNull CommandSender sender, @NotNull String arg2, @NotNull String[] args) {
        ...
        if (args.length == 1) {
            ...
            if (p.hasPermission("darksmc.tphere")) {
                if (!matchPlayer.isEmpty()) {
                    Player cel = matchPlayer.get(0);
                    if (!p.getUniqueId().equals(cel.getUniqueId())) {
                        cel.teleport(p.getLocation());
                        p.sendMessage("§a✔ §x§7§9§F§F§5§5Przeteleportowałeś do siebie §x§3§0§D§F§0§0" + cel.getDisplayName());
                    } else {
